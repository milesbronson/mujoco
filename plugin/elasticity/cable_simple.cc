...
// From Eq 7: Ωi = (2/li) * Im[q̄i * qi+1]
//
//   inputs:
//     quati      - orientation of the body i (world frame)
//     quati1     - orientation of the body i+1 (world frame)
//     li         - length of body segment i
//   outputs:
//     omega      - Darboux Vector
void DarbouxVector(mjtNum omega[3], const mjtNum quati[4],
                  const mjtNum quati1[4], mjtNum li){
  mjtNum quati_conj[4];
  mju_negQuat(quati_conj, quati);

  mjtNum quat_prod[4];
  mju_mulQuat(quat_prod, quati_conj, quati1);

  omega[0] = (2.0 / li) * quat_prod[1];
  omega[1] = (2.0 / li) * quat_prod[2];
  omega[2] = (2.0 / li) * quat_prod[3];
}

// Compute local stress: -K * (omega - omega0)
//   inputs:
//     stiffness   - material parameters [GJ, EI1, EI2, length]
...
Cable::Cable(const mjModel* m, mjData* d, int instance) {
  // parameters were validated by the factory function
  ...
  for (int b = 0; b < n; b++) {
    ...
    // compute omega0: Darboux vector at equilibrium
    if (prev[b] && flat != "true") {
      int ip = i + prev[b];
...
void Cable::Compute(const mjModel* m, mjData* d, int instance) {
  for (int b = 0; b < n; b++)  {
    ...
    // contribution from left edge (body b is the right segment)
    if (prev[b]) {
      int ip = i + prev[b];
      mjtNum li = stiffness[4*b+3];
    ...
    // contribution from right edge (body b is the left segment)
    if (next[b]) {
      int bn = b + next[b];
      int in = i + next[b];
